...
const ipnSecretKey = process.env.NOWPAYMENTS_IPN_SECRET_KEY as string;

// Maps payment amount to credits. This is a fallback and should be robust.
// A better approach would be to encode this in the order_description or another field.
...
export async function POST(req: NextRequest) {
  if (!ipnSecretKey || ipnSecretKey === '<REPLACE_ME_WITH_YOUR_KEY>') {
  ...
  try {
    ...
    const { payment_status, order_id, price_amount, actually_paid } = body;
    ...
    if (payment_status === 'finished') {
      ...
      // Determine credits to add. Using price_amount as a key.
      ...
      if (creditsToAdd > 0) {
        ...
        console.log(`✅ Successfully added ${creditsToAdd} credits to user ${userId} via NOWPayments.`);
      } else {
        console.warn(`🤷 No credits configured for amount: ${amountKey}`);
      }
    }

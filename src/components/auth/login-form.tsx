...
export function LoginForm({ onSignupClick }: LoginFormProps) {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    toast({ title: 'Attempting Log In', description: 'Please wait...' });
  }
  ...
  const handleGithubSignIn = async () => {
    ...
    try {
      await signInWithPopup(auth, provider);
      toast({ title: 'Success', description: 'Logged in with GitHub.' });
    } catch (error: any) {
      toast({
        title: 'Error',
        description: error.message,
  ...
  return (
    <Card className="w-full max-w-sm border-none shadow-none">
      ...
      <CardFooter className="text-center text-sm">
        ...
        <Link href={onSignupClick ? '#' : "/signup"} onClick={onSignupClick} className="underline">
          Sign up
        </Link>
      </CardFooter>
    </Card>
